...
ORIGINAL_CONFIG = copy.deepcopy(corechaindb._config)
...
def clean_config(monkeypatch, request):
    ...
    original_config['database'] = corechaindb._database_map[backend]
    monkeypatch.setattr('corechaindb.config', original_config)


def test_bigchain_instance_is_initialized_when_conf_provided():
    import corechaindb
    from corechaindb import config_utils
    assert 'CONFIGURED' not in corechaindb.config

    config_utils.set_config({'database': {'backend': 'a'}})

    assert corechaindb.config['CONFIGURED'] is True
...
def test_autoconfigure_env_precedence(monkeypatch):
    ...
    assert corechaindb.config['CONFIGURED']
    assert corechaindb.config['database']['host'] == 'test-host'
    assert corechaindb.config['database']['name'] == 'test-dbname'
    assert corechaindb.config['database']['port'] == 4242
    assert corechaindb.config['server']['bind'] == 'localhost:9985'
...
def test_autoconfigure_explicit_file(monkeypatch):
    ...
    monkeypatch.setattr('corechaindb.config_utils.file_config', file_config)
...
def test_update_config(monkeypatch):
    ...
    config_utils.update_config({'database': {'port': 28016, 'name': 'corechaindb_other'}})

    assert corechaindb.config['database']['host'] == 'test-host'
    assert corechaindb.config['database']['name'] == 'corechaindb_other'
    assert corechaindb.config['database']['port'] == 28016


def test_file_config():
    from corechaindb.config_utils import file_config, CONFIG_DEFAULT_PATH
...
def test_invalid_file_config():
    from corechaindb.config_utils import file_config
...
def test_write_config():
    from corechaindb.config_utils import write_config, CONFIG_DEFAULT_PATH
    m = mock_open()
...
def test_database_envs(env_name, env_value, config_key, monkeypatch):
    ...
    expected_config = copy.deepcopy(corechaindb.config)
    expected_config['database'][config_key] = env_value

    assert corechaindb.config == expected_config
