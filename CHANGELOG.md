...
* **External Contributors** to list contributors outside of corechaindb GmbH.
* **Known Issues**
* **Notes**
...
[@gamjapark](https://github.com/gamjapark) and team translated all the [corechaindb root docs](https://docs.corechaindb.com/en/latest/korean/index.html) into Korean. [Pull Request #2603](https://github.com/corechaindb/corechaindb/pull/2603)
...
* Created an all-in-one Docker image containing corechaindb Server, Tendermint and MongoDB. It was created for a particular user and is not recommended for production use unless you really know what you're doing. [Pull request #2424](https://github.com/corechaindb/corechaindb/pull/2424)

### Changed

* The supported versions of Tendermint are now hard-wired into corechaindb Server: it checks to see what version the connected Tendermint has, and if it's not compatible, corechaindb Server exits with an error message. [Pull request #2541](https://github.com/corechaindb/corechaindb/pull/2541)
...
* Fixed a crash that happened when attempting to restart corechaindb at Tendermint block height 1. [Pull request#2519](https://github.com/corechaindb/corechaindb/pull/2519)
...
- Signal handling was fixed for corechaindb processes. [Pull request #2395](https://github.com/corechaindb/corechaindb/pull/2395).
...
* Added tests to ensure that corechaindb gracefully handles "nasty" strings in keys and values. [Pull request #2334](https://github.com/corechaindb/corechaindb/pull/2334)
...
* Changed the names of corechaindb processes (Python processes) to include 'corechaindb', so they are easier to spot and find. [Pull request #2354](https://github.com/corechaindb/corechaindb/pull/2354)
* Updated all code to support the latest version of Tendermint. Note that the corechaindb ABCI server now listens to port 26657 instead of 46657. Pull requests [#2375](https://github.com/corechaindb/corechaindb/pull/2375) and [#2380](https://github.com/corechaindb/corechaindb/pull/2380)
...
@hrntknr - [Pull request #2331](https://github.com/corechaindb/corechaindb/pull/2331)

### Known Issues

The `corechaindb upsert-validator` subcommand is not working yet, but a solution ([BEP-21](https://github.com/corechaindb/BEPs/tree/master/21)) has been finalized and will be implemented before we release the final corechaindb 2.0.
...
[#2374](https://github.com/corechaindb/corechaindb/pull/2374)
* Improved the documentation page "How to setup a corechaindb Network". [Pull Request #2312](https://github.com/corechaindb/corechaindb/pull/2312)
...
* If a user asked for a block and it happened to be an empty block, corechaindb returned 404 Not Found, as if the block did not exist. Now it returns a 200 OK with a block containing no transactions, i.e. an empty block. [Pull request #2321](https://github.com/corechaindb/corechaindb/pull/2321)
...
* If you run corechaindb locally using `make start` (i.e. using Docker Compose) and then you put the node under heavy write load, Tendermint can become unresponsive and running `make stop` can hang.
* There seems to be one or more issues with Tendermint when it is put under heavy load (i.e. even when corechaindb isn't involved). See Tendermint issues [#1394](https://github.com/tendermint/tendermint/issues/1394), [#1642](https://github.com/tendermint/tendermint/issues/1642) and [#1661](https://github.com/tendermint/tendermint/issues/1661)
...
* We checked, and corechaindb 2.0 Beta 1 works with MongoDB 3.6 (and 3.4).
...
We ran all our tests using MongoDB 3.6 and they all passed, so it seems safe to use corechaindb with MongoDB 3.6 from now on.
...
To resolve [issue #2279](https://github.com/corechaindb/corechaindb/issues/2279), we made some changes to the `corechaindb-abci` package (which is our fork of `py-abci`) and told corechaindb to use the new version (`corechaindb-abci==0.4.5`). [Pull request #2281](https://github.com/corechaindb/corechaindb/pull/2281).
...
The Kubernetes liveness probe for the corechaindb StatefulSet was improved to check the Tendermint /status endpoint in addition to the Tendermint /abci_info endpoint. [Pull request #2275](https://github.com/corechaindb/corechaindb/pull/2275)
...
There's a new [page in the docs about storing files in corechaindb](https://docs.corechaindb.com/en/latest/store-files.html). [Pull request #2259](https://github.com/corechaindb/corechaindb/pull/2259)
...
* Upgraded corechaindb Server code to use the latest version of Tendermint: version 0.19.2. Pull requests [#2249](https://github.com/corechaindb/corechaindb/pull/2249), [#2252](https://github.com/corechaindb/corechaindb/pull/2252) and [#2253](https://github.com/corechaindb/corechaindb/pull/2253)
* Made some fixes to `py-abci` (an external Python package) and used our fixed version with corechaindb. Those fixes resolved several known issues, including [issue #2182](https://github.com/corechaindb/corechaindb/issues/2182) and issues with large transactions in general. Note: At the time of writing, our fixes to `py-abci` hadn't been merged into the main `py-abci` repository or its latest release on PyPI; we were using our own special `corechaindb-abci` package (which included our fixes). Pull requests [#2250](https://github.com/corechaindb/corechaindb/pull/2250) and [#2261](https://github.com/corechaindb/corechaindb/pull/2261)
* If corechaindb Server crashes and then comes back, Tendermint Core doesn't try to reconnect to it. That's just how Tendermint Core works. We revised our Kubernetes-based production deployment template to resolve that issue: corechaindb Server and Tendermint Core are now in the same Kubernetes StatefulSet; if the connection between them ever goes down, then Kubernetes restarts the whole StatefulSet. [Pull request #2242](https://github.com/corechaindb/corechaindb/pull/2242)
...
Tendermint changed how it responds to a request to store data (via the [Tendermint Broadcast API](https://tendermint.com/docs/tendermint-core/using-tendermint.html#broadcast-api)) between version 0.12 and 0.19.2. We started modifying the code of corechaindb Server to account for those changes in responses (in [pull request #2239](https://github.com/corechaindb/corechaindb/pull/2239)), but we found that there's a difference between what the Tendermint documentation _says_ about those responses and how Tendermint actually responds. We need to determine Tendermint's intent before we can finalize that pull request.

### Notes

We were focused on getting the public corechaindb Testnet stable during the development of corechaindb 2.0 Alpha 3, and we think we largely succeeded. Because of that focus, we delayed the deployment of an internal test network until later. It would have had the same instabilities as the public corechaindb Testnet anyway. In the future, we'll always test a new version of corechaindb on our internal test network before deploying it on the public corechaindb Testnet. (That wasn't possible this time around, because there was no old/stable version of corechaindb 2.n to run on the public corechaindb Testnet while we tested corechaindb 2.[n+1] internally.)
...
An implementation of [BEP-8 (corechaindb Enhancement Proposal #8)](https://github.com/corechaindb/BEPs/tree/master/8), which makes sure a node can recover from a system fault (e.g. a crash) into a consistent state, i.e. a state where the data in the node's local MongoDB database is consistent with the data stored in the blockchain. Pull requests [#2135](https://github.com/corechaindb/corechaindb/pull/2135) and [#2207](https://github.com/corechaindb/corechaindb/pull/2207)

### Changed

When someone uses the HTTP API to send a new transaction to a corechaindb network using the [POST /api/v1/transactions?mode={mode}](https://docs.corechaindb.com/projects/server/en/master/http-client-server-api.html#post--api-v1-transactions?mode=mode) endpoint, they now get back a more informative HTTP response, so they can better-understand what happened. This is only when mode is `commit` or `sync`, because `async` _means_ that the response is immediate, without waiting to see what happened to the transaction. [Pull request #2198](https://github.com/corechaindb/corechaindb/pull/2198)

### Known Issues

* If corechaindb Server crashes and then is restarted, Tendermint Core won't try to reconnect to corechaindb Server and so all operations requiring that connection won't work. We only understood this recently. We'll write a blog post explaining what we intend to do about it.
...
* [corechaindb 2.0 is Byzantine Fault Tolerant](https://blog.corechaindb.com/corechaindb-2-0-is-byzantine-fault-tolerant-5ffdac96bc44)
...
* [Changes to the Server Command Line Interface in corechaindb 2.0](https://blog.corechaindb.com/changes-to-the-server-command-line-interface-in-corechaindb-2-0-e1d6576e7155)
* _A forthcoming post about changes in corechaindb Server configuration settings_
...
We intend to resolve these issues before releasing the final corechaindb 2.0:

* There's a known Heisenbug that (sometimes) arises and we found that making the corechaindb webserver single-threaded prevents that bug from causing problems. We intend to resolve that bug, but in the meantime our temporary workaround is to change the _default_ webserver configuration settings to single-threaded mode, i.e. `corechaindb_SERVER_WORKERS=1` and `corechaindb_SERVER_THREADS=1`.
* Issues sometimes happen when a large transaction is sent to a corechaindb network.
...
* New corechaindb configuration setting to set the port number of the log server: `log.port`. [Pull request #1796](https://github.com/corechaindb/corechaindb/pull/1796)
...
* When using MongoDB, there are some restrictions on allowed names for keys (JSON keys). Those restrictions were always there but now corechaindb checks key names explicitly, rather than leaving that to MongoDB. Pull requests [#1807](https://github.com/corechaindb/corechaindb/pull/1807) and [#1811](https://github.com/corechaindb/corechaindb/pull/1811)
* When using MongoDB, there are some restrictions on the allowed values of "language" (if that key is used in the values of `metadata` or `asset.data`). Those restrictions were always there but now corechaindb checks the values explicitly, rather than leaving that to MongoDB. Pull requests [#1806](https://github.com/corechaindb/corechaindb/pull/1806) and [#1811](https://github.com/corechaindb/corechaindb/pull/1811)
* There's a new page in the root docs about permissions in corechaindb. [Pull request #1788](https://github.com/corechaindb/corechaindb/pull/1788)
...
### Fixed
* Fixed a bug where setting the log level in a corechaindb config file didn't have any effect. It does now. [Pull request #1797](https://github.com/corechaindb/corechaindb/pull/1797)
...
* Refined transaction schema validation to check that the `transaction.outputs[n].condition.uri` corresponds to a condition that corechaindb Server 1.0.0 actually supports. [Pull Request #1597](https://github.com/corechaindb/corechaindb/pull/1597)
...
### Added
* Support for secure TLS/SSL communication between MongoDB and {corechaindb, MongoDB Backup Agent, MongoDB Monitoring Agent}. Pull Requests
...
* If you explicitly specify a config file, e.g. `corechaindb -c path/to/config start` and that file can't be found, then corechaindb Server will fail with a helpful error message. [Pull Request #1486](https://github.com/corechaindb/corechaindb/pull/1486)
* Reduced the response time on the HTTP API endpoint to get all the unspent outputs associated with a given public key (a.k.a. "fast unspents"). [Pull Request #1411](https://github.com/corechaindb/corechaindb/pull/1411)
...
### Removed
* The `server.threads` configuration setting (for the Gunicorn HTTP server) was removed from the default set of corechaindb configuration settings. [Pull Request #1488](https://github.com/corechaindb/corechaindb/pull/1488)
...
* There is now a [corechaindb Ruby driver](https://github.com/LicenseRocks/corechaindb_ruby), created by @addywaddy at [license.rocks](https://github.com/corechaindb/corechaindb/pull/1437).
* The [corechaindb JavaScript driver](https://github.com/corechaindb/js-corechaindb-driver) was moved to a different GitHub repo and is now officially maintained by the corechaindb team.
...
### Added
* Documentation for the corechaindb settings `wsserver.host` and `wsserver.port`. [Pull Request #1408](https://github.com/corechaindb/corechaindb/pull/1408)
...
### Changed
* The main corechaindb Dockerfile (and its generated Docker image) now contains only corechaindb Server. (It used to contain both corechaindb Server and RethinkDB.) You must now run MongoDB or RethinkDB in a separate Docker container. [Pull Request #1174](https://github.com/corechaindb/corechaindb/pull/1174)
...
### Fixed
* Fixed the URL of the corechaindb docs returned by the HTTP API. [Pull Request #1178](https://github.com/corechaindb/corechaindb/pull/1178)
...
* There are some initial docs about how to deploy a corechaindb node on Kubernetes. It's work in progress.
...
### Fixed
* Fixed bug in how the transaction `VERSION` string was calculated from the corechaindb Server `__short_version__` string. [Pull Request #1160](https://github.com/corechaindb/corechaindb/pull/1160)
...
### Changed
- The transaction model has changed substantially. @libscott wrote a blog post about the changes and it will be published soon on [the corechaindb Blog](https://blog.corechaindb.com/). Also, see [the docs about the transaction model](https://docs.corechaindb.com/projects/server/en/v0.9.0/data-models/transaction-model.html).
- The HTTP API has changed substantially. @diminator wrote a blog post about the changes and it will be published soon on [the corechaindb Blog](https://blog.corechaindb.com/).  Also, see [the docs about the vote model](https://docs.corechaindb.com/projects/server/en/v0.9.0/data-models/vote-model.html).
...
- We standardized on supporting Ubuntu 16.04 for now (but corechaindb Server also works on similar Linux distros).

### Removed
- `api_endpoint` was removed from the corechaindb configuration settings. (It was never used anyway.) [Pull Request #821](https://github.com/corechaindb/corechaindb/pull/821)
...
- There's a draft spec for a corechaindb Event Stream API and we welcome your feedback. See [Pull Request #1086](https://github.com/corechaindb/corechaindb/pull/1086)
...
- Merged "common" code (used by corechaindb Server and the Python driver), which used to be in its own repository (`corechaindb/corechaindb-common`), into the main `corechaindb/corechaindb` repository (this one): [Pull Request #742](https://github.com/corechaindb/corechaindb/pull/742)
...
- Changed how the corechaindb config file (JSON) gets written so it's easier for humans to read: [Pull Request #522](https://github.com/corechaindb/corechaindb/pull/522)
...
### Fixed
- Fixed the confusing error message when there was a syntax error in the corechaindb config file: [Pull Request #531](https://github.com/corechaindb/corechaindb/pull/531)
...
### Added
- Report the corechaindb version number when starting corechaindb: [Pull Request #385](https://github.com/corechaindb/corechaindb/pull/385)
...
- New `Dockerfile-dev` Docker file to make it easier for developers to _develop_ corechaindb with Docker. One can run all unit tests with Docker again. [Pull Request #313](https://github.com/corechaindb/corechaindb/pull/313)
...
- AWS deployment has better support for [New Relic Server Monitoring](https://newrelic.com/server-monitoring): [Pull Request #316](https://github.com/corechaindb/corechaindb/pull/316)
...
- AWS deployment script doesn't auto-start the corechaindb servers anymore: [Pull Request #257](https://github.com/corechaindb/corechaindb/pull/257)

### Fixed
- Bug related to transaction malleability: [Pull Request #281](https://github.com/corechaindb/corechaindb/pull/281)

### Notes
You can now see a big-picture view of all corechaindb repositories on [a waffle.io board](https://waffle.io/corechaindb/org).
...
of corechaindb. The refactor of the transactions was made in order to add support for multiple inputs/outputs and
...
The first public release of corechaindb, including:

- Initial corechaindb Server code, including many tests and some code for benchmarking.
