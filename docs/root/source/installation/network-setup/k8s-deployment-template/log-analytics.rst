...
All logging messages from containers running the Docker image corechaindb/nginx_3scale:1.3, containing the string "GET" but not the strings "Go-http-client" or "runscope" (where those exclusions filter out tests by Kubernetes and Runscope):

``Type=ContainerLog Image="corechaindb/nginx_3scale:1.3" GET NOT("Go-http-client") NOT(runscope)``
