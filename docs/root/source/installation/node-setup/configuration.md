...
Every corechaindb Server configuration setting has two names: a config-file name and an environment variable name. For example, one of the settings has the config-file name `database.host` and the environment variable name `corechaindb_DATABASE_HOST`. Here are some more examples:

`database.port` ↔ `corechaindb_DATABASE_PORT`

`database.keyfile_passphrase` ↔ `corechaindb_DATABASE_KEYFILE_PASSPHRASE`

`server.bind` ↔ `corechaindb_SERVER_BIND`
...
The local config file is `$HOME/.corechaindb` by default (a file which might not even exist), but you can tell corechaindb to use a different file by using the `-c` command-line option, e.g. `corechaindb -c path/to/config_file.json start`
or using the `corechaindb_CONFIG_PATH` environment variable, e.g. `BIGHAINDB_CONFIG_PATH=.my_corechaindb_config corechaindb start`.
Note that the `-c` command line option will always take precedence if both the `corechaindb_CONFIG_PATH` and the `-c` command line option are used.

You can read the current default values in the file [corechaindb/\_\_init\_\_.py](https://github.com/corechaindb/corechaindb/blob/master/corechaindb/__init__.py). (The link is to the latest version.)

Running `corechaindb -y configure localmongodb` will generate a local config file in `$HOME/.corechaindb` with all the default values.
...
* `database.connection_timeout` is the maximum number of milliseconds that corechaindb will wait before giving up on one attempt to connect to the backend database.
* `database.max_tries` is the maximum number of times that corechaindb will try to establish a connection with the backend database. If 0, then it will try forever.
* `database.replicaset` is the name of the MongoDB replica set. The default value is `null` because in corechaindb 2.0+, each corechaindb node has its own independent MongoDB database and no replica set is necessary. Replica set must already exist if this option is configured, corechaindb will not create it.
* `database.ssl` must be `true` or `false`. It tells corechaindb Server whether it should connect to MongoDB using TLS/SSL or not. The default value is `false`.

There are three ways for corechaindb Server to authenticate itself with MongoDB (or a specific MongoDB database): no authentication, username/password, and x.509 certificate authentication.

**No Authentication**

If you use all the default corechaindb configuration settings, then no authentication will be used.

**Username/Password Authentication**

To use username/password authentication, a MongoDB instance must already be running somewhere (maybe in another machine), it must already have a database for use by corechaindb (usually named `bigchain`, which is the default `database.name`), and that database must already have a "readWrite" user with associated username and password. To create such a user, login to your MongoDB instance as Admin and run the following commands:
...
```text
export corechaindb_DATABASE_BACKEND=localmongodb
export corechaindb_DATABASE_HOST=localhost
export corechaindb_DATABASE_PORT=27017
export corechaindb_DATABASE_NAME=database8
export corechaindb_DATABASE_CONNECTION_TIMEOUT=5000
export corechaindb_DATABASE_MAX_TRIES=3
```

**Default values**

If (no environment variables were set and there's no local config file), or you used `corechaindb -y configure localmongodb` to create a default local config file for a `localmongodb` backend, then the defaults will be:
...
[External clients]---(port 443)---[NGINX]---(port 9984)---[Gunicorn / corechaindb Server]
...
```text
export corechaindb_SERVER_BIND=0.0.0.0:9984
export corechaindb_SERVER_LOGLEVEL=debug
export corechaindb_SERVER_WORKERS=5
```
...
```text
export corechaindb_WSSERVER_SCHEME=ws
export corechaindb_WSSERVER_HOST=0.0.0.0
export corechaindb_WSSERVER_PORT=9985
```
...
different from where corechaindb is running.

**Example using environment variables**

```text
export corechaindb_WSSERVER_ADVERTISED_SCHEME=wss
export corechaindb_WSSERVER_ADVERTISED_HOST=mycorechaindb.com
export corechaindb_WSSERVER_ADVERTISED_PORT=443
```
...
"wsserver": {
    ...
    "advertised_host": "mycorechaindb.com",
    "advertised_port": 443
}
```
...
{
    "log": {
        "file": "/var/log/corechaindb.log",
        "error_file": "/var/log/corechaindb-errors.log",
        "level_console": "info",
        "level_logfile": "info",
...
{
    "log": {
        "file": "~/corechaindb.log",
        "error_file": "~/corechaindb-errors.log",
        "level_console": "info",
        "level_logfile": "info",
        "datefmt_console": "%Y-%m-%d %H:%M:%S",
        "datefmt_logfile": "%Y-%m-%d %H:%M:%S",
...
The user running `corechaindb` must have write access to the
specified path.
...
Log levels for corechaindb's modules. This can be useful to control the log
...
The settings with names of the form `tendermint.*` tell corechaindb Server
...
```text
export corechaindb_TENDERMINT_HOST=tendermint
export corechaindb_TENDERMINT_PORT=26657
```
