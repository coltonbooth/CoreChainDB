...
Show help for the `corechaindb` command. `corechaindb -h` does the same thing.


## corechaindb \-\-version

Show the version number. `corechaindb -v` does the same thing.
...
```text
corechaindb -y configure localmongodb
```
...
If you want to force-drop the database (i.e. skipping the yes/no prompt), then use `corechaindb -y drop`
...
```bash
$ corechaindb --log-level INFO start
```
...
```bash
$ corechaindb election new upsert-validator <public-key> <power> <node-id> --private-key <path-to-the-private-key>
```
...
```bash
$ corechaindb election new upsert-validator HHG0IQRybpT6nJMIWWFWhMczCLHt6xcm7eP52GnGuPY= 1 fb7140f03a4ffad899fabbbf655b97e0321add66 --private-key /home/user/.tendermint/config/priv_validator.json
...
For more details about how validator set changes work, refer to [BEP-21](https://github.com/corechaindb/BEPs/tree/master/21).
...
```bash
$ corechaindb election new chain-migration --private-key <path-to-the-private-key>
```
...
```bash
$ corechaindb election new migration --private-key /home/user/.tendermint/config/priv_validator.json
[SUCCESS] Submitted proposal with id: 04a067582cf03eba2b53b82e4adb5ece424474cbd4f7183780855a93ac5e3caa
```
...
For more details about how chain migrations work, refer to [Type 3 scenarios in BEP-42](https://github.com/corechaindb/BEPs/tree/master/42).
...
Approve an election by voting for it. The command places a `VOTE` transaction, spending all of the validator's vote tokens to the election address.
 ...
 ```bash
$ corechaindb election approve <election-id> --private-key <path-to-the-private-key>
```
...
Example:
 ```bash
$ corechaindb election approve 04a067582cf03eba2b53b82e4adb5ece424474cbd4f7183780855a93ac5e3caa --private-key /home/user/.tendermint/config/priv_validator.json
...
```bash
$ corechaindb election show <election-id>
